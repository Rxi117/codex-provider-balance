# Codex 余额菜单补丁 · 分身版(codex-provider-balance)

**简体中文** | [English](README.en.md)

把正版 Codex **复制一份**成「分身」,只在分身里打补丁,把各家 API 余额显示到头像菜单的「剩余用量」下面:一个平台一行,每行带对应厂商的 logo,点击任意一行可以打开本机配置面板。分身和正版可以同时运行。
...
正版 Codex 是 WindowsApps 下的 MSIX 包,只读、带签名校验,改不了;所以方案是把正版**整份复制**到 `C:\CodexDeepSeekPatched`,只在副本里注入余额行。

- 副本**不会自己更新**:正版升级后,副本仍停在你复制那天的版本,它把来源版本号记在 `installed-version.txt`。
- 所以更新正版后,想要「新版 + 余额行」,就得**重新复制一份新版 → 再打一次补丁**;不重打的话,副本还是旧的,界面可能与新版对不上。
- **这一步已经自动化**:启动文件夹里的 `Codex Provider Balance Auto Update` 每次登录会静默运行 `install-user-patched-codex.ps1`,它对比正版当前版本与副本记录的版本,不一致(或补丁标记丢失)就自动重复制 + 重打补丁,一致则直接退出。正常情况不需要手动管。
- **真正的例外**:补丁靠匹配打包压缩后的函数名(如 `lGt`、`uGt`)定位注入点。Codex 来一次大版本重构、这些名字变了,自动脚本就找不到注入点、打不上补丁,这时需要人工重新分析新版 `app.asar`、重新定位注入点。这也是它属于「自己维护的本地魔改」而非一劳永逸成品的原因。
